...
puts "Réponse : #{journalists.select{ |i| i[/\d+/]}.length} handles contiennent un numéro"
...
puts "Réponse : Il y a #{journalists.grep(/@[[:upper:]]/).count} handles qui commencent par une majuscule juste après le @."

puts '-' * 10
puts"Combien contiennent au moins une majuscule ?"
puts "Réponse : Il y a #{journalists.grep(/[[:upper:]]/).count} handles qui contiennent au moins une majuscule."
...
journalists_sorted_by_length = journalists.sort_by {|object| object.length}
puts journalists_sorted_by_length[0..49]
